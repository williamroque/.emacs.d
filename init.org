...
Options for when running in a terminal.
...
Make tramp handle verification steps better.

#+begin_src emacs-lisp :tangle yes
(setq-default tramp-password-prompt-regexp "^.*\\(\\(Password\\|Verification\\)\\).*: ? *")
#+end_src

Reuse SSH connection for file operations.
...
Only check for Git remotely.

#+begin_src emacs-lisp :tangle yes
(setq-default vc-handled-backends '(Git))
#+end_src

Prevent auto-saves and backups over network.
...
Use =scp= when transferring large files.
...
Ensure evil-mode compatibility.
...
#+begin_src emacs-lisp :tangle no :noweb-ref org-roam-leader
...
#+begin_src text :tangle no :noweb-ref org-latex-mla-class
